...
- 예외필터는 `@UseFilter` 데커레이터로 컨트롤러에 직접 적용하거나 전역으로 적용할 수 있음.

## 인터셉터 Interceptor
- 요청과 응답을 가로채서 변형할 수 있음
- 미들웨어와 수행하는 일이 비슷하지만 수행시점이 다르다.

## 요청 생명주기
#### 미들웨어
- 전역으로 바인딩된 미들웨어 실행 -> 모둘에 바인딩되는 순서대로 실행

#### 가드
- 전역으로 바인딩된 가드 먼저 실행 -> 컨트롤러 정의된 순서대로 실행

#### 인터셉터
- 요청은 전역->컨트롤러->라우터 순서대로 동작하지만 응답은 라우터->컨트롤러->전역으로 동작

#### 파이프
- 파이프가 여러 레벨에 적용되어 있다는 다른것들과 마찬가지의 순서로 적용. 전역 -> 정의된 순서
- 파이프가 적용된 라우터의 매개변수가 여러 개 있을때는 정의한 순서의 역순으로 적용

#### 예외 필터
- 전역 필터가 먼저 적용되지 않음.
- 라우터->컨트롤러->전역으로 바인딩 된 순서대로 동작
...
## Clean Architecture
- 4개 분류
  - InfraStructure : 외부에서 가져다 쓰는 컴포넌트 ex) DB, 이메일전송, 다른 서비스랑 통신, 외부 라이브러리를 제공받아 애플리케이션에 맞춰 구현하는 구현체 포함
  - Interface : 서비스가 제공하는 인터페이스가 구현되는 레이어
  - Application : 비즈니스 로직이 구현되는 레이어. 유저를 예를들면 회원가입, 이메일 인증, 회원정보 조회하는 등의 로직들
  - Domain : 핵심 도메인을 구현하는 레이어. 애플리케이션이 가져야되는 핵심 요소만 가지고, 도메인 레이어가 변경되면 다른 모든 레이어를 수정해야함
